 ...
 * correctly.
 */
char misc[ROUND_UP(4096, PAGE_SIZE)]
    __attribute__((aligned(PAGE_SIZE)))
    __attribute__((externally_visible));

size_t region_misc_size(void) {
    return 4096;
}

int region_misc_read(void *dest, size_t size, off_t offset) {
#ifndef NO_BOUNDS_CHECKS
    if (offset + size > region_misc_size()) {
        return -1;
    }
#endif
    memcpy(dest, misc + offset, size);
    return 0;
}

int region_misc_write(void *src, size_t size, off_t offset) {
#ifndef NO_BOUNDS_CHECKS
    if (offset + size > region_misc_size()) {
        return -1;
    }
#endif
    memcpy(misc + offset, src, size);
    return 0;
}

void *region_misc_base(void) {
    return (void*)misc;
}
